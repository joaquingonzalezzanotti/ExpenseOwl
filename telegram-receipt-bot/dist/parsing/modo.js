...
export const parseModoReceipt = (text, aliases, telegramMeta) => {
    ...
    const motiveRaw = pick(clean, /Motivo\s+(.+?)(?:\s+Fecha y hora|\s+Fecha y Hora|$)/i);
    const bankMemo = isTechnicalBankMemo(motiveRaw) ? motiveRaw : undefined;
    const motive = bankMemo ? undefined : motiveRaw;
