...
function normalizeContextDate(value?: string): string {
  if (!value) {
...
function decodeBase64OrThrow(base64Value: string): Buffer {
...
function normalizeMediaInput(media: ParseRequest["media"]): NormalizedMediaInput | null {
  ...
  if (!SUPPORTED_MEDIA_MIME_TYPES.has(mimeType)) {
    throw new HttpError(
      400,
      `Unsupported media MIME type '${mimeType}'. Allowed: ${Array.from(SUPPORTED_MEDIA_MIME_TYPES).join(", ")}`,
    );
  }
...
function buildSystemInstruction(referenceDateISO: string): string {
...
function parseModelJSON(rawResponseText: string): unknown {
...
function normalizeType(raw: unknown): "income" | "expense" | "refund" | undefined {
...
function normalizeSourceApp(raw: unknown): "MODO" | "BANK" | "WALLET" | "UNKNOWN" {
  const value = String(raw || "").trim().toUpperCase();
  if (value === "MODO") return "MODO";
...
function inferTypeFromText(text?: string): "income" | "expense" | "refund" | undefined {
  ...
  if (!value) return undefined;
  if (/\b(reintegro|devolucion|cashback)\b/.test(value)) return "refund";
  if (/\b(cobre|recibi|ingrese|depositaron|me transfirieron)\b/.test(value)) return "income";
  if (/\b(gaste|pague|compre|pago)\b/.test(value)) return "expense";
  return undefined;
}

function inferAmountFromText(text?: string): number | undefined {
...
function inferCounterpartyFromText(text?: string): string | undefined {
  const value = String(text || "").trim();
  if (!value) return undefined;
  const toOrAt = value.match(/\b(?:en|a)\s+(.+)$/i)?.[1]?.trim();
  if (toOrAt) return toOrAt;
  return undefined;
}
...
function normalizeConfidence(raw: unknown, fallbackOverall: number): z.infer<typeof ConfidenceSchema> {
...
function normalizeModelOutput(raw: unknown, inputText: string | undefined, contextDateISO: string): z.infer<typeof TransactionSchema> {
  ...
  const counterparty = pickFirstNonEmptyString(
    source.counterparty,
    source.merchant,
    source.comercio,
    source.destination,
...
class AIService {
  ...
  async parse(input: {
    ...
    media?: NormalizedMediaInput | null;
  }): Promise<Transaction> {
    const parts = [];
    parts.push(
      createPartFromText(
        "Extrae el movimiento financiero y devuelve exclusivamente JSON con el contrato pedido.",
      ),
    );
...
app.listen(PORT, "0.0.0.0", () => {
  logger.info(
    { port: PORT, model: GEMINI_MODEL, maxMediaBytes: MAX_MEDIA_BYTES },
    "ExpenseLog AI Parser running",
  );
});
